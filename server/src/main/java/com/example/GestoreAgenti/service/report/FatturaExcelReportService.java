...
public class FatturaExcelReportService {
    ...
    public byte[] generaReport(List<Fattura> fatture) {
        ...
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            EnumMap<InvoiceState, List<Fattura>> fatturePerStato = new EnumMap<>(InvoiceState.class);
            for (Fattura fattura : fatture) {
                if (fattura == null) {
                    continue;
                }
                InvoiceState stato = resolveState(fattura);
                fatturePerStato.computeIfAbsent(stato, key -> new ArrayList<>())
                        .add(fattura);
            }

            if (fatturePerStato.isEmpty()) {
                createEmptySheet(workbook, headerStyle);
            } else {
                for (InvoiceState stato : InvoiceState.values()) {
                    SheetContext context = createSheet(workbook, stato, headerStyle);
                    List<Fattura> perStato = fatturePerStato.get(stato);
                    if (perStato != null && !perStato.isEmpty()) {
                        for (Fattura fattura : perStato) {
